...
func main() {
	...
	fmt.Println("test.....")
	test0_msg := `{"id":22,"name":"Annual Fee9 cuijing" }`
	test0_msg2 := []string{`{"id":66,"name":"Annual Fee9 cuijing" }`, `{"id":22,"name":"Annual Fee9 cuijing" }`}
	fmt.Println(test0_msg)
	...
	client, err := elastic.NewClient(elastic.SetURL("http://192.168.253.161:9200"))
	...
	info, code, err := client.Ping("http://192.168.253.161:9200").Do(context.TODO())
	if err != nil {
		// Handle error
		panic(err)
	}
	...
	esversion, err := client.ElasticsearchVersion("http://192.168.253.161:9200")
	if err != nil {
		// Handle error
		panic(err)
	}
	...
	exists, err := client.IndexExists("products").Do(context.TODO())
	if err != nil {
		// Handle error
		panic(err)
	}
	...
	// cuijing20170901  bulk test Index a product
	bulkRequest := client.Bulk()
	//var index1Req *elastic.BulkIndexRequest
	for _, v := range test0_msg2{
		indexReq := elastic.NewBulkIndexRequest().Index("products1").Type("test").Doc(v)
		bulkRequest = bulkRequest.Add(indexReq)
	}
	bulkResponse, err := bulkRequest.Do(context.TODO())
	if err != nil {
		fmt.Println("test20170901_01", err)
	}
	if bulkResponse == nil {
	...
	if bulkRequest.NumberOfActions() != 0 {
		fmt.Println("expected bulkRequest.NumberOfActions %d; got %d", 0, bulkRequest.NumberOfActions())
	}
